      ...
      <TabsList className="flex flex-wrap justify-center gap-2 bg-transparent h-auto p-0 mb-8">
        {itineraries.map((it) => (
          <TabsTrigger
            ...
            className="data-[state=active]:bg-nepal-crimson data-[state=active]:text-white font-[family-name:var(--font-space-grotesk)] rounded-full px-4 py-2 border border-nepal-mist data-[state=active]:border-nepal-crimson data-[state=active]:shadow-md data-[state=active]:shadow-nepal-crimson/20"
          >
            {it.label}
          </TabsTrigger>
        ))}
      ...
      {itineraries.map((itinerary) => (
        <TabsContent key={itinerary.duration} value={String(itinerary.duration)}>
          <div className="relative">
            {/* Timeline Line */}
            <div className="absolute left-6 top-0 bottom-0 w-0.5 bg-nepal-crimson/20 hidden sm:block" />
            ...
            <div className="space-y-4">
              {itinerary.days.map((day, idx) => (
                ...
                >
                  {/* Day Badge */}
                  <div className="flex-shrink-0 w-12 h-12 bg-nepal-crimson text-white rounded-full flex items-center justify-center font-[family-name:var(--font-space-grotesk)] font-bold text-sm relative z-10">
                    D{day.day}
                  </div>

                  {/* Content Card */}
                  <div className="flex-1 bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-nepal-mist/50 hover:shadow-md transition-shadow">
                    <h4 className="font-[family-name:var(--font-playfair)] font-bold text-nepal-slate">
                      {day.title}
                    </h4>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="font-[family-name:var(--font-space-grotesk)] text-sm font-medium text-nepal-slate mb-2 block">
            Trip Duration
          </label>
      ...
      {estimate && (
        <motion.div
          ...
          className="bg-gradient-to-br from-nepal-crimson/5 to-nepal-gold/5 rounded-2xl p-6 sm:p-8 border border-nepal-crimson/10"
        >
          <div className="text-center">
            <div className="font-[family-name:var(--font-space-grotesk)] text-sm text-muted-foreground mb-2">
              Estimated Budget
            </div>
            <div className="font-[family-name:var(--font-playfair)] text-3xl sm:text-4xl font-bold text-nepal-crimson">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {essentials.map((item, idx) => (
        <motion.div
          ...
          className="bg-white rounded-xl p-5 border border-nepal-mist/50 shadow-sm"
        >
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 bg-nepal-crimson/10 rounded-lg flex items-center justify-center">
              <item.icon className="h-5 w-5 text-nepal-crimson" />
            </div>
            <h4 className="font-[family-name:var(--font-space-grotesk)] font-semibold text-nepal-slate">
              {item.title}
            </h4>
          </div>
    ...
    <Accordion type="multiple" className="w-full">
      {Object.entries(packingChecklist).map(([category, items], idx) => (
        <AccordionItem
          key={category}
          value={category}
          className="border-nepal-mist/50"
        >
          <AccordionTrigger className="font-[family-name:var(--font-space-grotesk)] font-medium text-nepal-slate hover:text-nepal-crimson hover:no-underline">
            <div className="flex items-center gap-3">
              <Backpack className="h-4 w-4 text-nepal-gold" />
              {category}
              <Badge
                variant="secondary"
                className="bg-nepal-mist text-xs font-[family-name:var(--font-space-grotesk)]"
              >
                {items.length} items
              </Badge>
            </div>
          ...
          <AccordionContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2">
              {items.map((item, i) => (
                ...
                >
                  <div className="w-1.5 h-1.5 rounded-full bg-nepal-crimson flex-shrink-0" />
                  {item}
                </div>
              ))}
            </div>
    ...
    <Accordion type="single" collapsible className="w-full">
      {travelFAQs.map((faq, idx) => (
        <AccordionItem
          ...
          className="border-nepal-mist/50"
        >
          <AccordionTrigger className="font-[family-name:var(--font-playfair)] text-left text-nepal-slate hover:text-nepal-crimson hover:no-underline text-base">
            {faq.question}
          </AccordionTrigger>
          <AccordionContent>
            <p className="text-sm text-muted-foreground leading-relaxed">
...
export default function TripPlannerSection() {
  ...
  return (
    <section id="plan" className="py-20 sm:py-28 bg-nepal-white">
